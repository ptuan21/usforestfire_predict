#!/usr/bin/env python3
"""
Dashboard US Wildfire Prediction — Plotly Dash (giao dien sang, chuyen nghiep).
Chay:  python3 src/dashboard_dash.py   ->  http://127.0.0.1:8050
Du lieu nhe da tinh san o data/processed/dashboard/ (chay precompute_dashboard_data.py truoc).
"""
...
fig_mon=style_fig(px.bar(mon,x="month",y="n",template=TEMPLATE,title="Phan bo theo thang (mua chay)",
...
fig_state=style_fig(px.choropleth(by_state,locations="STATE",locationmode="USA-states",color="n",
    ...
    title="So vu chay theo bang"),h=460)
...
app.layout=html.Div(style={"backgroundColor":C_BG,"minHeight":"100vh","color":C_TEXT,
    ...
    html.Div(style={"maxWidth":"1180px","margin":"0 auto","padding":"0 24px"},children=[
        dcc.Tabs(id="tabs",value="risk",children=[
            dcc.Tab(label="Ban do rui ro (California)",value="risk",style=TABSTY,selected_style=TABSEL),
            dcc.Tab(label="Tong quan toan quoc",value="nat",style=TABSTY,selected_style=TABSEL),
            dcc.Tab(label="Hieu nang mo hinh",value="model",style=TABSTY,selected_style=TABSEL)]),
...
def render(tab):
    if tab=="risk":
        return html.Div(style={"display":"flex","gap":"18px","flexWrap":"wrap"},children=[
            html.Div(style={**CARD,"flex":"1","minWidth":"260px","maxWidth":"320px"},children=[
                html.Div("Lua chon",style={"fontWeight":"600","marginBottom":"10px"}),
                html.Label("Nam",style={"color":C_MUTE,"fontSize":"13px"}),
                dcc.Dropdown(id="year",options=[{"label":str(y),"value":int(y)} for y in YEARS],
                             value=int(YEARS[-1]),clearable=False),
                html.Div(style={"height":"12px"}),
                html.Label("Tuan",style={"color":C_MUTE,"fontSize":"13px"}),
                dcc.Dropdown(id="week",clearable=False),
                html.Div(style={"height":"14px"}),
                html.Label("Nguong canh bao",style={"color":C_MUTE,"fontSize":"13px"}),
    ...
    if tab=="nat":
        ...
        return html.Div([
            html.Div(f"Tong {tot:,} vu chay — FPA FOD (1992-2020) + NIFC (2021-2026)",
    ...
    return html.Div([
        html.Div([html.H3("Chi so cac mo hinh",style={"marginTop":"0"}),
...
def upd_map(wk,thr):
    ...
    fig.update_layout(paper_bgcolor=C_PANEL,font_color=C_TEXT,margin=dict(l=0,r=0,t=4,b=0),
                      coloraxis_colorbar=dict(title="Rui ro"),
    ...
    cards=[metric_card("O co chay",f"{nf}"),metric_card("Rui ro TB",f"{d['risk'].mean():.2f}"),
           metric_card("O canh bao",f"{flagged}"),metric_card("Bat duoc",f"{recall*100:.0f}%")]
    note=html.Span([html.B(f"{top20*100:.0f}%"),
        f" trong 20 o rui ro cao nhat tuan nay thuc su co chay."])
