...
Giao diện trực quan hoá: bản đồ rủi ro cháy California chọn theo tuần, tổng quan cháy toàn quốc, hiệu năng mô hình.

```bash
pip install streamlit plotly
python3 src/precompute_dashboard_data.py   # tạo dữ liệu nhẹ cho dashboard (cần model + panel)
streamlit run src/dashboard.py             # mở http://localhost:8501
```

3 trang: **Bản đồ rủi ro** (chọn năm/tuần → bản đồ xác suất cháy + cháy thực tế, ngưỡng cảnh báo), **Tổng quan toàn quốc** (số vụ/diện tích theo năm, nguyên nhân, mùa, bản đồ theo bang), **Hiệu năng mô hình**.
